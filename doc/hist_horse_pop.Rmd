...
- Horses, number on farms at June 1 and at December 1 [@Horses1]
- Horses, number on farms at June 1, farm value per head and total farm value [@Horses2]

## Methods

The R programming language [@R] and the following R packages were used to perform the analysis: knitr [@knitr], tidyverse [@tidyverse] and caret [@caret]. The code used to perform the analysis and create this report can be found here: https://github.com/ttimbers/equine_numbers_value_canada_rmd.


## Results

```{r plot horses, fig.width=10, fig.height=3, fig.cap = "Figure 1. Historical number of horses per province in Canada", out.width="100%", out.height="100%"}
...
max_horses <- horse_pop %>% 
  ...
  summarize(Maximum = max(Value),
            Minimum = min(Value))
kable(max_horses, caption = "Table 1. Range of the number of horses for each provinces between 1940 - 1972.")
```

Below we zoom in and look at the province of `r params$province`:

```{r plot province, fig.width=5, fig.height=4, fig.cap = "Figure 2. Historical number of horses per province in Canada.", out.width="100%", out.height="100%"}
